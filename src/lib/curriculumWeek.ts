 ...
 * actually limited is the item bank: only Weeks 1-2 have real practice
 * questions written for their newly-introduced skills so far (see
 * src/db/seed/week1-item-bank.json, week2-item-bank.json). Advancing
 * MAX_AVAILABLE_WEEK further without matching item content would leave a
 * child with "new" skills that have no items to practise — raise it only
 * once each additional week's item bank exists.
 */
